...
namespace Twitch.Stream.App
{
   class DownloadVod : IApp
   {
      ...
      {
         var tasks = _options.Streams.Select(async videoId =>
         {

            var videos = await _apiTwitch.GetVideoInfoAsync(videoId);
            ...
            var vodTwitchAuth = await _apiTwitch.GetVodTwitchAuthAsync(videoId);
